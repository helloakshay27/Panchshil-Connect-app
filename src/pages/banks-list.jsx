...
const BankList = () => {
  ...
  const getPageFromStorage = () => {
    return parseInt(localStorage.getItem("bank_currentPage")) || 1;
  };
  ...
  const getBankPermissions = () => {
    try {
      const lockRolePermissions = localStorage.getItem("lock_role_permissions");
      if (!lockRolePermissions) return {};
      return JSON.parse(lockRolePermissions).bank || {};
    } catch (e) {
  ...
  useEffect(() => {
    const permissions = getBankPermissions();
    setBankPermissions(permissions);
  }, []);
    ...
    const fetchBanks = async () => {
      ...
      try {
        ...
        setPagination((prevState) => ({
          ...
          total_pages: Math.ceil(banksData.length / itemsPerPage),
          current_page: getPageFromStorage(),
        }));
      } catch (error) {
  ...
  const handleToggle = async (id, currentStatus) => {
    try {
      ...
      await axios.put(
        `${baseURL}banks/${id}.json`,
        { bank: { active: updatedStatus } },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access_token")}`,
            "Content-Type": "application/json",
  ...
  const displayedBanks = filteredBanks.slice(
      ...
      <div className="module-data-section container-fluid">
        ...
        <div className="card mt-3 mx-3">
          <div className="card-header">
            <h3 className="card-title">Banks List</h3>
          </div>
                  ...
                  <tbody>
                    {displayedBanks.length > 0 ? (
                      displayedBanks.map((bank, index) => (
                        <tr key={bank.id}>
                          <td>
                            {/* {bankPermissions.update === "true" && ( */}
                              <button
                                className="btn btn-link p-0"
                                onClick={() =>
                                  navigate(`/setup-member/banks/${bank.id}/edit`)
                                }
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                          ...
                          <td>{bank.interest_rate ? `${bank.interest_rate}%` : "-"}</td>
                          <td>
                            {bank.bank_logo ? (
                              <img
                                src={bank.bank_logo}
