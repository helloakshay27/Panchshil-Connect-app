...
const UserGroupCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${baseURL}usergroups.json`, requestData, {
        headers: {
      ...
      navigate("/setup-member/usergroup-list"); // Adjust navigation path as needed
    } catch (error) {
  ...
  onChange={selectedOptions =>
    setFormData(prev => ({
      ...prev,
      member_ids: selectedOptions.map(option => option.value),
    }))
  }
/>
                                             </div>
                    ...
                    <div className="row mt-3">
                      <div className="col-12">
                        <div className="alert alert-info">
                          <strong>Selected Members ({formData.member_ids.length}):</strong>
                          <div className="mt-2">
                            {formData.member_ids.map(memberId => {
                              const user = users.find(u => u.id.toString() === memberId.toString());
                              return (
                                <span key={memberId} className="badge badge-primary mr-2 mb-1">
                                  {user ? `${user.firstname} ${user.lastname}` : `User ${memberId}`}
                                </span>
                              );
                            })}
                          </div>
