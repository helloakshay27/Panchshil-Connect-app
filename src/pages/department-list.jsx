...
const DepartmentList = () => {
  ...
  const getPageFromStorage = () => {
    return parseInt(localStorage.getItem("department_list_currentPage")) || 1;
  };
  ...
  const getDepartmentPermission = () => {
    try {
      const lockRolePermissions = localStorage.getItem("lock_role_permissions");
      if (!lockRolePermissions) return {};

      const permissions = JSON.parse(lockRolePermissions);
      return permissions.department || {}; // Fetching department-specific permissions
    } catch (e) {
  ...
  useEffect(() => {
    const permissions = getDepartmentPermission();
    console.log("Department permissions:", permissions);
    setDepartmentPermission(permissions);
  }, []);
  ...
  useEffect(() => {
    const fetchDepartmentList = async () => {
      ...
      try {
        ...
        // Check if response.data is an array (direct list of departments)
  ...
  const handlePageChange = (pageNumber) => {
    ...
    localStorage.setItem("department_list_currentPage", pageNumber);
  };
    ...
    <div className="main-content">
      <div className="module-data-section container-fluid">
        <div className="d-flex justify-content-end px-4 pt-2 mt-3">
          ...
          {/* {departmentPermission.create === "true" && ( */}
                        ...
                        <tr key={department.id}>
                          ...
                          <td>
                                {/* {userPermission.destroy === "true" && ( */}
                                  <button
                                    onClick={() =>
                                      handleToggleUser(department.id, department.active)
                                    }
                  ...
                  <p>
                    {pagination.total_count > 0 ? (
                      <>
                        ...
                        {Math.min(
                          startIndex + displayedDepartments.length,
