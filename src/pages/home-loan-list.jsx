...
const HomeLoanList = () => {
  ...
  console.log("home loan permission:", homeLoanPermissions);
  ...
  // Toggle status function (if needed for home loans)
  const onToggle = async (loanId, currentStatus) => {
    toast.dismiss();
    setLoading(true);
    try {
      const response = await axios.put(
        `${baseURL}home_loans/${loanId}.json`,
        {
          home_loan: {
      ...
      console.error("Error toggling home loan status:", error);
      toast.error("Failed to update home loan status. Please try again.");
    } finally {
      setLoading(false);
    }
  };

  // Filter home loans based on search query
            ...
            {/* {homeLoanPermissions.create === "true" && ( */}
              <div className="card-tools">
                <button
                  ...
                  onClick={() => navigate("/setup-member/home-loan-create")}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={26}
          ...
          <div className="module-data-section container-fluid">
            <div className="card mt-3 mx-3">
              <div className="card-header">
                <h3 className="card-title">Home Loan List</h3>
              </div>
              <div className="card-body mt-3 pb-4 pt-0">
                        ...
                        {displayedHomeLoans.length > 0 ? (
                          displayedHomeLoans.map((loan, index) => (
                            <tr key={loan.id}>
                              ...
                              <td>
                                {loan.project?.Project_Name || "N/A"}
                              </td>
                              <td>
                                {loan.project?.project_address || "N/A"}
                              </td>
                              <td>
                          ...
                          <tr>
                            <td colSpan="8" className="text-center">
                              No home loans found matching your search criteria
                            </td>
                          </tr>
                        )}
                      </tbody>
                ...
                {!loading && displayedHomeLoans.length > 0 && (
