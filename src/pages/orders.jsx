...
const Orders = () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (response.data.pagination) {
          ...
          setTotalPages(
            Math.ceil(response.data.pagination.total_count / perPage),
          );
        }
  ...
  const updateOrderStatus = async (orderId, newStatus, notes = "") => {
    try {
      ...
      const url = `${baseURL}orders/${orderId}/status_update.json?status=${newStatus}&notes=${encodeURIComponent(
        notes,
      )}`;
      await axios.put(
        url,
  ...
  const tableData = Array.from({ length: totalCount }, (_, index) => {
    const pageStart = (currentPage - 1) * perPage;
    return (
      orders[index - pageStart] || {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Created At",
      sortable: false,
      render: (order) => <small>{formatDate(order.created_at)}</small>,
    },
  ];
