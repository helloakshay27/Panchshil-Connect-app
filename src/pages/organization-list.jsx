...
const OrganizationList = () => {
  const [organizations, setOrganizations] = useState([]); // Ensure it's an array
    ...
    fetch("https://panchshil-super.lockated.com/organizations.json", {
      ...
      .then((data) => {
        ...
        setPagination({
          ...
          total_pages: Math.ceil(orgs.length / pageSize),
        });
      })
      .catch((error) => {
        console.error("Error fetching organizations:", error);
        setOrganizations([]); // Prevent `.filter()` error
      });
  }, []);
  ...
  // ✅ Ensure `organizations` is an array before filtering
            ...
            <div className="card-body mt-4 pb-4 pt-0">
              ...
              <div className="d-flex justify-content-between align-items-center px-3 mt-2">
                <ul className="pagination justify-content-center d-flex">
                  ...
                  <li className={`page-item ${pagination.current_page === 1 ? "disabled" : ""}`}>
                    <button className="page-link" onClick={() => handlePageChange(pagination.current_page - 1)} disabled={pagination.current_page === 1}>Prev</button>
                  </li>
                  ...
                  <li className={`page-item ${pagination.current_page === totalPages ? "disabled" : ""}`}>
                    <button className="page-link" onClick={() => handlePageChange(pagination.current_page + 1)} disabled={pagination.current_page === totalPages}>Next</button>
                  </li>
                  <li className={`page-item ${pagination.current_page === totalPages ? "disabled" : ""}`}>
                    <button className="page-link" onClick={() => handlePageChange(totalPages)} disabled={pagination.current_page === totalPages}>Last</button>
                  </li>
                </ul>
                <p>Showing {(pagination.current_page - 1) * pageSize + 1} to {pagination.current_page * pageSize} of {totalFiltered} entries</p>
              </div>
            </div>
          </div>
        </div>
